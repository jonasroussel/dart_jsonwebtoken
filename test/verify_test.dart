...
void main() {
  group('Verify a JWT', () {
    ...
    group('Claims', () {
      ...
      group('aud', () {
        test('list audience from JSON-decoded token passes validation', () {
          final jwt = JWT(
            {'foo': 'bar'},
            audience: Audience(['api', 'web']),
          );
          final signed = jwt.sign(hsKey, noIssueAt: true);
          ...
          expect(verified.payload['foo'], equals('bar'));
          expect(verified.audience, equals(['api', 'web']));
        });
        test('list audience mismatch fails validation', () {
          final jwt = JWT(
            {'foo': 'bar'},
            audience: Audience(['api', 'web']),
          );
